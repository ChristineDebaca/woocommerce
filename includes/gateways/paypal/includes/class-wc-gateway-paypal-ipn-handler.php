...
class WC_Gateway_Paypal_IPN_Handler extends WC_Gateway_Paypal_Response {
	 ...
	 */
	public function __construct( $sandbox = false, $receiver_email = '', $paymentaction = '' ) {
	...
	public function valid_response( $posted ) {
		if ( ! empty( $posted['custom'] ) && ( $order = $this->get_paypal_order( $posted['custom'] ) ) ) {
			...
			// Sandbox fix.
			if ( 'authorization' !== $this->paymentaction && isset( $posted['test_ipn'] ) && 1 == $posted['test_ipn'] && 'pending' == $posted['payment_status'] ) {
